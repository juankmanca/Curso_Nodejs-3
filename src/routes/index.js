...
app.get('/Coordinador/VerCursos', (req, res) => {
    Usuario.findById(req.session.usuario, (err, usuario) => {
        ...
        let txt = '';
        Relacion.find({}).exec((err, resultado) => {
            if (err) {
                return console.log(err);
            }
            console.log(resultado)
            resultado.forEach(element => {
                Curso.findOne({ id: element.cursoId }, function(err, resultados) {
                    if (err) {
                        return console.log(err)
                    }
                    ...
                    txt += `<div class="card-body">
                      ...
                      Usuario.findOne({ cedula: element.cedula }, (err, resultados) => {
                          if (err) {
                              return console.log(err)
                          }
                          txt += `<input type="text" value="${element._id}" name="_id" hidden>
                          <input type="submit" value="Eliminar" class="btn btn-danger">
                      <p class="card-text">${resultados.nombre}</p>
                      </div>`
                      txt += `</div></form>`
